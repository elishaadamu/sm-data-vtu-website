...
import NecoStudents from "@/assets/neco-students.jpeg";
...
const NecoPage = () => {
  ...
  const [examType, setExamType] = useState("token");
  const [quantity, setQuantity] = useState(1);
  const [phoneNumber, setPhoneNumber] = useState("");
  const [email, setEmail] = useState("");

  const cardPrice = 1200; // default NECO Scratch Card/Token price
    ...
    MySwal.fire({
      ...
      html: `
        <div class="text-left space-y-2 text-slate-700">
          <p>You are about to purchase <strong>${quantity} NECO Token(s)</strong>.</p>
          <div class="bg-slate-50 p-4 rounded-xl border space-y-1 text-sm">
            <div class="flex justify-between"><span>Exam:</span> <strong>NECO</strong></div>
          ...
          <p class="text-xs text-slate-500 mt-2">Tokens will be sent to <strong>${phoneNumber}</strong></p>
        </div>
      `,
      icon: "info",
      ...
      if (result.isConfirmed) {
        ...
        try {
          ...
          const response = await axios.post(
            apiUrl(API_CONFIG.ENDPOINTS.NECO.BUY_PIN),
            payload
          );
          ...
          MySwal.fire({
            ...
            text: response.data?.message || "NECO Token purchased successfully!",
          ...
          MySwal.fire({
            ...
            text: error.response?.data?.message || "Failed to purchase NECO Token. Please try again.",
      ...
      <div className="relative overflow-hidden rounded-3xl bg-slate-900 text-white p-6 md:p-8 mb-8 shadow-xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-3 max-w-lg">
            <span className="bg-teal-600/30 text-teal-300 border border-teal-500/30 px-3 py-1 rounded-full text-xs font-semibold tracking-wider uppercase">
              Education Services
            </span>
            <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">
              NECO Scratch Card / Token
            </h1>
            <p className="text-teal-100 text-sm md:text-base leading-relaxed">
              Buy NECO Result Checker Tokens instantly. Fast, automated processing & secure delivery to your phone.
            </p>
          </div>
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-3xl p-6 md:p-8 shadow-lg">
            <form onSubmit={handlePurchase} className="space-y-6">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all outline-none text-slate-800 text-xs sm:text-sm font-medium appearance-none"
                    required
                  >
                    <option value="token">NECO Result Checker Token</option>
                  </select>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="sm:hidden relative">
                  <select
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all outline-none text-slate-800 text-sm font-medium"
                  >
                    {[1, 2, 3, 4, 5].map((q) => (
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">
                  Phone Number (for Token delivery)
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-transparent transition-all outline-none text-slate-800"
                    required
                  />
                </div>
              </div>
              ...
              <div className="bg-teal-50/70 border border-teal-100 p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-sm">
                <div>
                  ...
                  <p className="text-slate-800 text-sm font-medium mt-1">
                    ₦{cardPrice.toLocaleString()} × {quantity} token(s)
                  </p>
                </div>
                <div className="text-left sm:text-right border-t sm:border-t-0 pt-2 sm:pt-0 border-teal-100/50">
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-700 hover:to-emerald-700 text-white font-bold py-4 rounded-xl shadow-lg hover:shadow-teal-500/20 transform hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed text-lg flex items-center justify-center gap-2"
              >
                {loading ? (
                  <>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-3xl p-6 shadow-sm">
            ...
            <ol className="space-y-3 text-sm text-slate-600 list-decimal pl-4">
              <li>Visit the NECO Result portal (results.neco.gov.ng).</li>
              <li>Select your Exam Year.</li>
              <li>Select your Exam Type (e.g. June/July or Nov/Dec).</li>
              <li>Enter the Token purchased here.</li>
              <li>Enter your Registration Number.</li>
              <li>Click "Check Result" and wait for your result page to display.</li>
            </ol>
          </div>
          ...
          <div className="bg-yellow-50/50 border border-yellow-200 rounded-3xl p-6 shadow-sm">
            ...
            <p className="text-xs text-yellow-700/95 leading-relaxed">
              NECO Token is used to check NECO result for one candidate. Ensure your registration number is correct before clicking search on the NECO portal.
            </p>
          </div>
        </div>
      </div>
