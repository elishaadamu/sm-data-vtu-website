...
export const fetchAdminStats = async (adminId) => {
  try {
    ...
    console.error("Error fetching admin stats:", error.response || error);
...
export const fetchAdminDailyStats = async (adminId) => {
  try {
    ...
    console.error("Error fetching daily stats:", error.response || error);
    return null;
  }
};
...
export const fetchAdminWeeklyStats = async (adminId) => {
  try {
    ...
    console.error("Error fetching weekly stats:", error.response || error);
    return null;
  }
};
...
export const fetchAdminUsers = async (adminUserId) => {
  try {
    ...
    console.error("Error fetching admin users:", error.response || error);
...
export const fetchUserStats = async (userId, adminUserId) => {
  try {
    ...
    console.error("Error fetching user stats:", error.response || error);
    return null;
  }
};
...
export const apiAddBalance = async (adminUserId, { phone, userId, amount }) => {
  try {
    ...
    console.error("Error adding balance:", error.response || error);
    throw error;
  }
};
...
export const apiDebitBalance = async (adminUserId, { phone, userId, amount }) => {
  try {
    ...
    console.error("Error debiting balance:", error.response || error);
    throw error;
  }
};
...
export const apiUpgradeUser = async (adminUserId, userId) => {
  try {
    ...
    console.error("Error upgrading user:", error.response || error);
    throw error;
  }
};
...
export const apiDowngradeUser = async (adminUserId, userId) => {
  try {
    ...
    console.error("Error downgrading user:", error.response || error);
    throw error;
  }
};

export const apiSendNotification = async (adminUserId, { title, message, userIds = [], targetAll }) => {
  try {
    const endpoint = `${API_CONFIG.ENDPOINTS.ADMIN.SEND_NOTIFICATION}${adminUserId}`;
    return await adminFetch(apiUrl(endpoint), {
      method: "POST",
      body: JSON.stringify({ title, message, userIds, targetAll }),
    });
  } catch (error) {
    throw error;
  }
...
export const fetchAdminNotifications = async (adminUserId) => {
  try {
    const endpoint = `${API_CONFIG.ENDPOINTS.NOTIFICATIONS.GET_ADMIN}${adminUserId}`;
    return await adminFetch(apiUrl(endpoint), {
      method: "GET",
    });
  } catch (error) {
...
export const apiMarkNotificationRead = async (notificationId, userId) => {
  try {
    const endpoint = `${API_CONFIG.ENDPOINTS.NOTIFICATIONS.READ}${notificationId}/mark-read/${userId}`;
    return await adminFetch(apiUrl(endpoint), {
      method: "POST",
    });
  } catch (error) {
...
export const apiDeleteNotification = async (notificationId, adminUserId) => {
  try {
    const endpoint = `${API_CONFIG.ENDPOINTS.NOTIFICATIONS.DELETE}${notificationId}/${adminUserId}`;
    return await adminFetch(apiUrl(endpoint), {
      method: "DELETE",
    });
  } catch (error) {
...
export const fetchAdminDailyProfit = async (adminId) => {
  try {
    const endpoint = `${API_CONFIG.ENDPOINTS.ADMIN.PROFIT_DAILY}${adminId}`;
    return await adminFetch(apiUrl(endpoint));
  } catch (error) {
...
export const fetchAdminWeeklyProfit = async (adminId) => {
  try {
    const endpoint = `${API_CONFIG.ENDPOINTS.ADMIN.PROFIT_WEEKLY}${adminId}`;
    return await adminFetch(apiUrl(endpoint));
  } catch (error) {
...
export const fetchAdminMonthlyProfit = async (adminId) => {
  try {
    const endpoint = `${API_CONFIG.ENDPOINTS.ADMIN.PROFIT_MONTHLY}${adminId}`;
    return await adminFetch(apiUrl(endpoint));
  } catch (error) {
...
export const getAdminMockProfits = () => {
  const orders = getAdminOrders();
  const today = new Date();

  const daily = Array.from({ length: 7 }, (_, i) => {
    const d = subDays(today, 6 - i);
    return { date: d.toISOString(), profit: 0 };
  });

  const weekly = Array.from({ length: 4 }, (_, i) => {
    const start = subDays(today, (4 - i) * 7);
    const end = subDays(today, (3 - i) * 7);
    return { week: `${format(start, "yyyy-MM-dd")} to ${format(end, "yyyy-MM-dd")}`, profit: 0 };
  });

  const monthly = Array.from({ length: 6 }, (_, i) => {
    const d = subDays(today, (6 - i) * 30);
    return { month: format(d, "MMMM"), profit: 0 };
  });
  ...
  orders.forEach((o) => {
    ...
    const diffDays = Math.floor(Math.abs(today - orderDate) / (1000 * 60 * 60 * 24));
    const value = parseInt(o.amount.replace(/[^0-9]/g, "")) || 0;
    const profitVal = Math.round(value * 0.1); // mock profit is 10% of sale

    if (diffDays < 7) {
      daily[6 - diffDays].profit += profitVal;
    }
    const weekIndex = Math.floor(diffDays / 7);
    if (weekIndex < 4) {
      weekly[3 - weekIndex].profit += profitVal;
    }
    const monthIndex = Math.floor(diffDays / 30);
    if (monthIndex < 6) {
      monthly[5 - monthIndex].profit += profitVal;
    }
  });
