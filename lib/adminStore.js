...
export const getAdminStats = () => {
  ...
  // Let's consider anyone with balance > 0 or recent transaction active
  ...
  // Daily / Weekly Logic
  const dailySales = [0, 0, 0, 0, 0, 0, 0]; // Last 7 days
  const weeklySales = [0, 0, 0, 0]; // Last 4 weeks
  ...
  orders.forEach((o) => {
    ...
    const amtStr = o.amount.replace(/[^0-9]/g, "");
    const value = parseInt(amtStr) || 0;

    if (diffDays < 7) {
      dailySales[6 - diffDays] += value;
    }
    
    const weekIndex = Math.floor(diffDays / 7);
    if (weekIndex < 4) {
      weeklySales[3 - weekIndex] += value;
    }
  });
  ...
  return {
    ...
    totalFunds,
    newUsersToday,
    dailySales, // Array of last 7 days values
    weeklySales, // Array of last 4 weeks values
  };
};
